 ...
 **/
public class FetchFromBiQuGe18 {

    public static void main(String[] args) {
        String articleCode = "/book/63698/";
    ...
    public static boolean fetchNovel(String articleName, String articleCode) {
        ...
        String url = SearchWeb.BIQUGE18.getWebUrl().concat(articleCode);
        //消除不受信任的HTML(防止XSS攻击)
    ...
    private static ContentDTO getContent(String uri, String titleName, Integer id) {
        String url = SearchWeb.BIQUGE18.getWebUrl().concat(uri);
        //消除不受信任的HTML(防止XSS攻击)
        url = CommonUtil.transferToSafe(url);
    ...
    private static List<Title> getTitles(Document doc) {
        ...
        Elements elements = doc.getElementsByClass("box_con").get(1).select("dd").select("a");
        int count = 0;
